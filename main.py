...
def login_validation():
    ...
    cursor.execute("""SELECT * FROM `user` WHERE `username` LIKE '{}' AND `password` LIKE '{}' AND `user_type`='Admin'"""
...
def add_admin():
     ...
     cursor.execute("""INSERT INTO `user`(`user_id`, `fullname`, `username`, `email`,`address`, `password`,`user_type`) VALUES (NULL,'{}','{}','{}','{}','{}','{}')""".format(fullname,username,email,address ,password,usertype))
     conn.commit()
     cursor.execute("""SELECT * FROM `user` WHERE `username` LIKE '{}' and `user_type`='Admin'""".format(username,usertype))
     myadmin=cursor.fetchall()
     cursor.execute("""SELECT * FROM `user` WHERE `username` LIKE '{}' and `user_type`='User'""".format(username, usertype))
...
def insert():
    if request.method == "POST":
        ...
        cursor.execute("INSERT INTO apartment_details (Apartment_name,Location,Number_of_room,Floor,Type,Details,Description,Cost,image1,image2,image3,Available) VALUES (%s,%s,%s,%s, %s, %s, %s ,%s, %s, %s, %s,%s)", (apartmentname,location,numberofroom,floor,type,details,description,cost,filename1 ,filename2,filename3,available))
        conn.commit()
        return redirect('/apartment')
...
def update():
    if request.method == "POST":
        ...
        """, (apartmentname,location,numberofroom,floor,type,details,description,cost,filename1,filename2,filename3,available,apartmentno))
...
def insert_re():
    if request.method == "POST":
        ...
        cursor.execute( "INSERT INTO tenant_details (Tenant_name,Apartment_no,Address,Email,Phone_number) VALUES (%s, %s ,%s, %s, %s)",(tenantname,apartmentno,address,email,phonenumber))
        conn.commit()
...
def insert_rere():
    if request.method == "POST":
        ...
        cursor.execute("INSERT INTO rent_info (Tenant_ID,Tenant_name,Apartment_no,Paid_Date,Paid_amount,Transection_no,Month) VALUES (%s, %s,%s,%s,%s,%s,%s)",
...
def insert_rent():
    if request.method == "POST":
        ...
        cursor.execute( "INSERT INTO rent_info (Tenant_ID,Tenant_name,Apartment_no,Paid_Date,Paid_amount,Transection_no,Month) VALUES (%s, %s,%s,%s,%s,%s,%s)",(tenantid,tenantname,apartmentno,paiddate,paidamount,tran,month))
        conn.commit()
...
def send_request():
    if request.method == "POST":
        flash("Request for Apartment has been send from User")
        ...
        phonenumber = request.form['Phone_number']
        cursor.execute( "INSERT INTO request_details (Name,Apartment_no,Address,Email,Phone_number) VALUES (%s, %s ,%s, %s, %s)",(name,apartmentno,address,email,phonenumber))
        conn.commit()
...
def search():
    if request.method == "POST":
        ...
        cursor.execute("SELECT * FROM apartment_details WHERE Location LIKE %s OR Floor LIKE %s OR Number_of_room LIKE %s OR Cost LIKE %s ", (apartment,apartment,apartment,apartment))
...
def profile():
    if 'username' in session: 
       ...
       cursor.execute("SELECT * FROM tenant_details where email=(SELECT email FROM user WHERE username ='" + s + "')")
       t = cursor.fetchone()
...
def i_payrent():
    if request.method == "POST":
        ...
        cursor.execute( "INSERT INTO rent_request (Tenant_ID,Tenant_name,Apartment_no,Paid_Date,Paid_amount,Transection_no,Month) VALUES (%s, %s, %s,%s,%s,%s,%s)",(tenantid,tenantname,apartmentno,paiddate,paidamount,tran,month))
        conn.commit()
